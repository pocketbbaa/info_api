package com.kg.platform.common.utils.sensitivefilter;
...
public class SensitiveFilter implements Serializable {
     ...
     * 默认的单例，使用自带的敏感词库
     * 
     * public static final SensitiveFilter DEFAULT = new SensitiveFilter( new
    ...
    public String filter(String sentence/* , char replace */) {
        ...
        while (i < sp.length - 2) {
            ...
            if (node != null) {
                /*
                 * 如果能拿到第一个节点， 才计算mix（mix相同表示2个字符相同）。
                    ...
                    if (node.headTwoCharMix == mix) {
                        ...
                        if (desSet != null) {
                            for (StringPointer word : desSet.descendingSet()) {
                                /*
                                 * 仍然需要再判断一次，例如"色情信息哪里有？"， 如果节点只包含"色情电影"一个词，
                                 * 仍然能够取到word为"色情电影"，但是不该匹配。
                                 */
                                if (sp.nextStartsWith(i, word)) {
                                    // times++;
                                    logger.info("匹配到的敏感词：{}", word);
